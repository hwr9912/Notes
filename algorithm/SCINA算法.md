...
根据标记基因确定每个轮廓细胞j的细胞类型。默认情况下，假设标记基因是均为上调的。与其他细胞类型相比，如果在一种细胞类型中具有某个标记基因是低表达的，那么可以构建一个伪基因，其表达是该标记基因表达的倒数，因此该伪基因在该细胞类型中的伪表达量较高，可以看作标记基因。每个细胞，$j$，都属于$R$个细胞类型之一或其他细胞类型（新的未知细胞类型，即$z_j=0$）。每一种$r$类型的细胞所有的签名基因，均包含于$S_r$之中。同时假定未知的细胞类型没有激活任何基因特征。
